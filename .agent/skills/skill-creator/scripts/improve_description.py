...
using Google ADK with extended thinking.
"""
...
# Add parent directory to path to allow 'from scripts.utils import'
...
def improve_description(
    ...
    prompt = f"""You are optimizing a skill description for a Google Antigravity skill called "{skill_name}". A "skill" is sort of like a prompt, but with progressive disclosure -- there's a title and description that the agent sees when deciding whether to use the skill, and then if it does use the skill, it reads the .md file which has lots more details and potentially links to other resources in the skill folder like helper files and scripts and additional documentation or examples.
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    thinking_text = "" # Not directly available from gemini CLI output
    text = ""

    try:
        # Construct the gemini command
        cmd = ["gemini", "--output-format", "json", "--prompt", prompt]
        if model:
        ...
        if process.returncode != 0:
            print(f"Error calling gemini: {process.stderr}", file=sys.stderr)
            return current_description
            
        try:
            # Parse the JSON response
            # gemini CLI sometimes outputs lines before the JSON (like 'Loaded cached credentials.')
            stdout = process.stdout
            if "{" in stdout:
                stdout = stdout[stdout.find("{"):]
            response_json = json.loads(stdout)
            
            # Extract content from the specific gemini CLI JSON structure
            ...
            print(f"Error parsing gemini output: {e}\nOutput was: {process.stdout}", file=sys.stderr)
            return current_description
            
    except Exception as e:
        print(f"Exception during gemini call: {e}", file=sys.stderr)
        return current_description
    ...
    if len(description) > 1024:
        ...
        shorten_thinking = "" # Not directly available from gemini CLI output
        shorten_text = ""
        try:
            cmd = ["gemini", "--output-format", "json", "--prompt", f"{prompt}\n\nAssistant: {text}\n\nHuman: {shorten_prompt}"]
            if model:
                cmd.extend(["--model", model])
            ...
            if process.returncode == 0:
                ...
                if "{" in stdout:
                    stdout = stdout[stdout.find("{"):]
                shorten_response_json = json.loads(stdout)
                
                shorten_text = shorten_response_json.get("response", text)
            else:
                print(f"Error calling gemini for shortening: {process.stderr}", file=sys.stderr)
                shorten_text = text # Fallback to original if shortening fails
        except Exception as e:
            print(f"Exception during shortener call: {e}", file=sys.stderr)
            shorten_text = text # Keep original text on failure
